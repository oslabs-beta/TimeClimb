...
interface stepFunctionController {
    listStateMachines: (req: Request, res: Response, next: NextFunction) => void | Promise<void>;
}

const stepFunctionController: stepFunctionController = {
    //make request for all state machines from AWS
    listStateMachines: async function(req: Request, res: Response, next: NextFunction){
